...
import styles from "./ReportImport.module.css";
...
const ReportImport = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const labels = res.map(item => item.productName || "Sản phẩm không tên");
      const datas = res.map(item => item.importQuantity ?? 0);
    ...
    <Layout>
      <div className={styles["reportImport-container"]}>
        <div className={styles["RI-frame"]}>
          <h2 className={styles["reportImport-h2"]}>BIỂU ĐỒ BÁO CÁO NHẬP KHO</h2>
